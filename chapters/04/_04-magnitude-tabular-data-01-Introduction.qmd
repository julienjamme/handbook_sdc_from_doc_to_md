...
Example: Turnover (number of respondents) by Region and Industry

\begin{tabular}{l|rrrr}
& Industry A & Industry B & $\cdots$ & Total \\ \hline
Region 1 & 540 & 231 & $\cdots$ & \\
\end{tabular}
...
At first sight, one might find it difficult to understand how the kind of summary information published in magnitude tables presents a \index{disclosure risk} at all. However, it often occurs that cells of a table relate to a single or to only a few respondents. The number of this kind of small cells in a table will increase, the more grouping variables are used to specify the table, the higher the amount of detail provided by the grouping variables, and the more uneven the distributions of respondents over the categories of the grouping variables.

If a table cell relates to a small group of respondents (or even only one), then publication of the cell value may imply a \index{disclosure risk}. This is the case if these respondents could be identified by an intruder using information displayed in the table.
...
Assume that a table displays the sum of a variable "Production" by three subsectors of an economy sector. Assume that this sum is sensitive for one of the subsectors and that the table is protected by \index{cell suppression}, meaning that the confidential cell value is suppressed.
...
In @sec-secondary-protection-methods we introduce the concepts of secondary tabular data protection methodologies. The focus will be on \index{cell suppression}, but we will also mention other methodologies.

The software package τ‑ARGUS (see Hundepool et al., 2005) provides software tools for disclosure protection methods for tabular data. @sec-tau-argus-suppression is concerned with the practical implementation of secondary \index{cell suppression} as offered by τ‑ARGUS. In @sec-setting-up-a-tabular-data-protection-problem-in-practice we discuss information loss concepts as well as table structures considered by τ‑ARGUS. We compare the performance of different algorithms for secondary \index{cell suppression} in @sec-evaluation-of-secondary-cell-suppression-algorithms-offered-by-τargus and give software recommendations. In @sec-efficient-protection-process we explain how to set up procedures for tabular data protection in a practical way and give an introductive example in @sec-introductive-example. In @sec-methodology-seconday-suppression-tau-argus we briefly introduce the methodological concepts of the secondary \index{cell suppression} algorithms provided by τ-ARGUS. The chapter ends with @sec-CTA, introducing Controlled Tabular Adjustment as new emerging protection technique for magnitude tables which could become an alternative to the well-established \index{cell suppression} methodologies.
