...
### Application to Frequency Tables {#sec-CKM_freq}
...
At this point, it must be pointed out that if, in addition to frequencies, magnitudes and mean values are also published, the mean values should rather not be shown as original values, since otherwise there is a risk that the corresponding original frequency values can be disclosed. 


### Publication of mean values
In this section, we will explain why original means should rather not be published when the Cell Key Method is applied to frequency counts. We will illustrate this with an example and show ways to publish the mean values in a safer variant.
...
For information how to use the software, we refer to the vignettes of the respective R-packages (on CRAN), to the manual of  \targus \cite{taumanual} and to the quick references for CKM in \targus \cite{QuickRefTau42x}.
