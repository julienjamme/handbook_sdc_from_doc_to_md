...
Due to the range of data that can be used to identify the individual one can distinguish two types of \index{disclosure risk} for a dataset obtained once the SDC process has been applied (cf. Młodak, Pietrzak and Józefowski (2022)): 
...
Internal risk seems to be easier to compute than the external risk. Internal threats for confidentiality can be modelled by violation of the aforementioned rules based on frequency of combinations of values of categorical variables and of observations falling into an re-identification precision interval around a given value of continuous variable. However, the estimation of external risk requires a knowledge about possible alternative data sources available for the user. This knowledge is hard to obtain, but we can (with large probability) suppose which possibilities in this respect he/she can have. For instance, if the user is employed in the labour office, one can suppose the he/she has an access to the basis of unemployed persons, which can be linked by him/her with the database from the Labour Force Survey obtained from the official statistics or similar data holder. The internal and external risk can be combined to obtain a total \index{disclosure risk}.   

The key aim of the Statistical Disclosure Control is to achieve optimal balance between minimization of \index{disclosure risk} and simultaneous minimization of the 
information loss arising from the SDC process (what is equivalent to maximization of data utility for the possible users). So, both aspects of this problem will 
be described and critically discussed in this subchapter. We will present types of \index{disclosure risk}, show how one can measure of \index{disclosure risk} for categorical 
variables and continuous variables. Because usually for each of such types separate measures are applied, we investigate also a possibility of complex measurement 
of \index{disclosure risk} taking them jointly into account. A similar approach was also applied to measures of information loss: types of information loss and its measures 
for categorical and continuous variables are presented and next complex measures providing synthetic information in this respect are discussed. They are available 
already in the literature and implemented to the specialized statistical software. Finally, some remarks on the practical realization of trade-off between safety and 
utility of microdata are collected.
...
Due to the range of data that can be used to identify the individual one can distinguish two types of \index{disclosure risk} for a dataset obtained once the SDC process has been 
...
the official statistics or similar data holder. The internal and external risk can be combined to obtain a total \index{disclosure risk}.   

Another classification of \index{disclosure risk} is connected with the reference object. That is, the following types of risk from this point of view are distinguished 
(cf. Templ (2017)):
...
- global risk - the aggregated \index{disclosure risk} for the whole data set.

The statistician involved in processing and dissemination of data should estimate \index{disclosure risk} at each stage of the SDC process. It allows to track changes and 
...
\index{disclosure risk} can contribute to re-identification of individuals, it should be, however, confidential itself and cannot be known by the user.  

### Measures of disclosure risk for categorical variables

The key measures of \index{disclosure risk} for categorical variables in microdata are, in general, based on the frequency rules in the internal dimension. 
...
Shlomo (2022) proposes measures \index{disclosure risk} in synthetic data based on the comparison of the overall distributions in the original data versus synthetic data and 
...
Therefore, different solutions for measurement of \index{disclosure risk} have to be be applied. The famous approach in this context is used in the sdcMicro 
...
- probabilistic \index{record linkage} (Jaro (1989)): the \index{disclosure risk} is here understood as percentage of "linked" pairs of records from the original and protected datasets, 
...
It is worth noting that the classical construction of measures of \index{disclosure risk} is focused on the development of separate tools for categorical and continuous variables. 
As it was indicated earlier it is justified by their different nature. However, using them means that the actual \index{disclosure risk} may be underestimated. 
...
on the \index{disclosure risk} is usually confidential) are interested in obtaining precise, reliable and comprehensive information on the \index{disclosure risk}. 
...
Recall, that this approach takes all types of variables (according to their measurement scales) into account. In this way, the \index{disclosure risk} has been assessed in context of 
...
external risk. A similar idea can be used also to complex assessment of internal \index{disclosure risk} using the distance-based approach, where application of the analogous concept of 
distance allows for joint estimation of change of the risk before and after performing SDC process. Also the probabilistic \index{record linkage}, where the conditional probability that a 
...
also possible to use a comprehensive measure of \index{disclosure risk}, achieving a balance between minimizing these two quantities will become much easier.
...
(but the higher the \index{disclosure risk}! ). This requires pairing records in the original dataset and records in the protected dataset. 
...
is discussed for the following methods: local suppression, global recoding and \index{PRAM}. Entropy is an information-theoretic measure, but can be used in SDC if the 
...
As noted earlier, \index{PRAM} is a method that generalizes noise addition, suppression and recoding methods. Therefore, our description of the use of entropy will be limited to \index{PRAM}.

Let $V$ be a variable in the original dataset and $V'$ be the corresponding variable in the \index{PRAM}-protected dataset. 
Let $P_{V,V'} = \left\{ p\left( V' = j \mid V = i \right) \right\}$ be the \index{PRAM} Markov matrix. Then, the conditional uncertainty of $V$ given that $V' = j$ is:
$$
H\left( V \mid V' = j \right) = - \sum\limits_{i = 1}^{n}{p\left( V = i \mid V' = j \right)\log p}\left( V = i \mid V' = j \right) 
$${#eq-conditional-uncertainty} 

The probabilities in @eq-conditional-uncertainty can be derived from $P_{V,V'}$ using Bayes' formula. Finally, the entropy-based information loss measure $EBIL$ is obtained by accumulating expression ([-@eq-conditional-uncertainty]) for all individuals $r$ in the protected dataset $G$
$$
...
Trottini (2003) argues that, since information loss is to be traded off for \index{disclosure risk} and the latter is bounded ---there is no risk higher than 100\%---, upper bounds should be enforced for information loss measures. In practice, the proposal in Trottini (2003) is to limit those measures in @tbl-loss-information based on the mean variation to a predefined maximum value.
...
- establishing arbitrarily maximum allowable level of \index{disclosure risk} and minimize the information loss in this situation - it defends, first of all, 
the data confidentiality and trust to data holder in terms of privacy protection,
- establishing arbitrarily maximum allowable level of information loss and minimize the \index{disclosure risk} in this situation - it defends, first of all, 
...
Dandekar, R., Domingo-Ferrer, J., and Sebé, F., (2002). *LHS-based hybrid microdata vs. \index{rank swapping} and microaggregation for numeric microdata protection.* In J. Domingo-Ferrer, editor, Inference Control in Statistical Databases, volume 2316 of LNCS, pages 153--162, Berlin Heidelberg, 2002. Springer.
...
Sayers, A., Ben-Shlomo, Y., Blom, A. W., & Steele, F. (2016). Probabilistic \index{record linkage}. *International Journal of Epidemiology*, 45(3), 954-964.
...
Taylor, L., Zhou, X.-H., & Rise, P. (2018). A tutorial in assessing \index{disclosure risk} in microdata. *Statistics in Medicine*, 37 (25), 3693–3706.
